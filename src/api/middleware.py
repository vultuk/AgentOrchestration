...
class ErrorSanitizationMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        request.state.error_sanitization_active = True
        try:
            response = await call_next(request)
            response.headers.setdefault("X-Error-Sanitized", "0")
            return response
        except Exception as exc:
            logger.error(
                "Unhandled API exception sanitized",
                extra={"error_type": type(exc).__name__},
            )
            return JSONResponse(
                status_code=500,
                content={"detail": "Internal server error"},
                headers={"X-Error-Sanitized": "1"},
            )
        finally:
            if hasattr(request.state, "error_sanitization_active"):
                delattr(request.state, "error_sanitization_active")
...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        protected_path = request.url.path.startswith("/api/v2")
        token_path = request.url.path == "/api/v2/auth/token"
        if protected_path and not token_path:
